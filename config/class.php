...
class promo
{
	...
	function ambil($id_promo)
	{

		$ambil = $this->koneksi->query("SELECT * FROM tbl_promo WHERE 	id_promo='$id_promo'");
		$pecah = $ambil->fetch_assoc();
		return $pecah;
	}
	...
	function edit($kode,$tgl,$diskon,$foto,$ket,$id)
	{
		...
		if (empty($namagambar)) 
		{
			$this->koneksi->query("UPDATE tbl_promo SET kode_promo='$kode',tgl_promo='$tgl',diskon='$diskon',keterangan='$ket' WHERE id_promo='$id'");
		}
		else
		{
			move_uploaded_file($lokasi, "../promo/$namagambar");
			$this->koneksi->query("UPDATE tbl_promo SET kode_promo='$kode',tgl_promo='$tgl',diskon='$diskon',foto_promo='$namagambar',keterangan='$ket' WHERE id_promo='$id'");
		}
...
class produk
{
	...
	function add($id_kategori,$nama_produk,$harga_produk,$berat_produk,$stok_produk,$gambar_produk,$deskripsi_produk)
	{
		$id_user = $_SESSION['user']['id_user'];
		...
		else
		{
			...
			$this->koneksi->query("INSERT INTO tbl_produk (id_kategori,nama_produk,harga_produk,berat_produk,deskripsi_produk,gambar_produk,stok,id_user) VALUES ('$id_kategori','$nama_produk','$harga_produk','$berat_produk','$deskripsi_produk','$namagambar','$stok_produk','$id_user')")or die(mysqli_error($this->koneksi));
			return "sukses";
		}
...
class penjualan
{
	...
	function transaksi_masuk($tgl_masuk,$nama_pelanggan,$total_belanja){

		$this->koneksi->query("INSERT INTO tbl_data_pengiriman (tanggal_pembelian,status_pembelian,total_pembelian,nama_penerima)
			VALUES ('$tgl_masuk','Selesai','$total_belanja','$nama_pelanggan')");
